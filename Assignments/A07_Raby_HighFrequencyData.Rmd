...
KansasRiver.Desoto.all.gathered <- gather(KansasRiver.Desoto.all, variable, value, "DO_mgL":"Nitrate_mgL")
...
KansasRiver.Desoto.baseflow <- EcoHydRology::BaseflowSeparation(
  na.omit(KansasRiver.Desoto.all$Discharge_m3s), 
  filter_parameter = 0.925, 
  passes = 3
  )

KansasRiver.Desoto.Discharge <- KansasRiver.Desoto.all %>%
  select("DateTime_Solar", "SiteName", "Discharge_m3s") %>%
  drop_na(Discharge_m3s) %>%
  cbind(KansasRiver.Desoto.baseflow)

KansasRiver.Desoto.Discharge.gathered <- gather(KansasRiver.Desoto.Discharge, Discharge_type, "value_m3s", "Discharge_m3s":"qft")


ggplot(KansasRiver.Desoto.Discharge.gathered) + 
  geom_line(aes(x = DateTime_Solar, y = value_m3s, color = Discharge_type)) +
  labs(x = "", y = expression("Discharge (m"^3*"/s)"), title = "Kansas River at Desoto Total Flow, Baseflow, and Quickflow Over Time") +
