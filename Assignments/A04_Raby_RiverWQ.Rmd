...
O2.plot <- ggplot(O2.dat, aes(x = Date, y = O2_mgL)) +
  ...
  geom_text(x = as.Date("1990-06-01"), y = 2.3, label = "hypoxia cutoff", hjust = 1, fontface = "bold") +
...
> In the period of time of the data, hypoxia was a very common phenomena at the beginning of the studied period. We can see several hypoxia measurements in late summer, beginning of fall periods of years 1983 and 1984. After that there are no more recorded hypoxia events.
...
Nitrogen.plot <- ggplot(nutrients.dat, aes(x = Date)) +
  ...
  labs(x = "", y = expression("Total Nitrogen mg/l")) 
...
dygraph(DyDat) %>% 
  dySeries("O2.Concentration", axis = "y2") %>% 
  dyAxis(name = "y", label = "Discharge (cfs)", valueRange = c(0, 200)) %>%
  dyAxis(name = "y2", label = "O2 Concentration (mg/L)", valueRange = c(0, 4)) %>%
  dyRangeSelector()
...
> My conclusion were fundamentally based on the amazing plots that we had to make for this lab. DO, Nutrients and Discharge vs Time. 
