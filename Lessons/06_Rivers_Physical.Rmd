...
2. Analyze the effects of watershed disturbance on recurrence intervals and interpret results against the concept of stationarity (climate change, phisical manipulation of the watershed, etc)
...
>Longer dry periods. More intensity. Water warmer affects ecosystems. Storage changes. Changes on statistical parameters that were used to design things. 

>Change in seasonal patterns. Less predictable.  Decreasing snowpacks. Hydropower. Impact the timing of the rains. Changes on freezings. Problem with fruits.
...
>not moving target. The conditions are not changing. You can predict.
...
>The USGS uses 30 years of data for recurrence intervals. Golden number
...
EnoRecurrence <- 
  ...
  mutate(Rank = rank(-PeakDischarge), #from 0ne to thirty. the negative gives the direction
...
summary(Eno.RImodel) #log because we saw the data in a graph.
...
> Having more data is useful. The full data includes higher discharge periods. Therefore, the coeficients are higher. The full data has a steeper slope.
...
> To include all the data available. 
Moving windows. You include some nonstationary information.
...
SaladoPeakPlot2 <- 
  ggplot(SaladoRecurrence2, aes(x = Year, y = PeakDischarge)) +
         geom_bar(stat = "identity") +
         xlab("Year")
print(SaladoPeakPlot2)

Salado2RecurrencePlot <- 
  ggplot(SaladoRecurrence2, aes(x = RecurrenceInterval, y = PeakDischarge)) +
  geom_point() #+
  #scale_x_log10() 
print(Salado2RecurrencePlot)

Salado2.RImodel <- lm(data = SaladoRecurrence, PeakDischarge ~ log(RecurrenceInterval))
summary(Salado2.RImodel) 
...
SaladoRecurrencePlot.Full <- 
  ggplot(SaladoRecurrence, aes(x = RecurrenceInterval, y = PeakDischarge)) +
  geom_point() +
  geom_point(data = SaladoRecurrence2, color = "#02818a",
             aes(x = RecurrenceInterval, y = PeakDischarge))
