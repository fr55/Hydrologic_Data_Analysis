...
> Toxins. Smell, color. Things harmfull to humans of aquatic life.
DO
Nutrients (N, P, Trace nutrients)
Microbes
Odor/Color
Invasive species
Contaminants
...
options(scipen = 100) # Prevents scientific notation.

# lagosne_get(dest_folder = LAGOSNE:::lagos_path(), overwrite = TRUE)
# you only need this once. Didnt work. We have a back up. 
```
...
```{r}
# Load LAGOSNE data into R session #FRA: didnt work because the up thing didnt work.
LAGOSdata <- lagosne_load()
names(LAGOSdata)

# If the package installation and data download has not worked, use this code: 
load(file = "./Data/Raw/LAGOSdata.rda") #FRA: we ran this because the official way didnt work.
...
# Tell R to treat lakeid as a factor, not a numeric value FRA:so we dont have problems
...
# Order by number of lakes FRA: for bar plots.
LAGOSlocations <- 
...
FRA: It can be a proxy to other parameters. (hyperuttophic) looking for changes in trophic states can be an indicatr or other things
...
LAGOStrophic <- 
  ...
  drop_na(chla:secchi) #month 12.  Winter is going to be 1. including december. Kateri made the decision. You can break your seasonality the way it make sense to you.


levels(LAGOStrophic$season) <- c("Winter", "Spring", "Summer", "Fall") #we name the numbers of the quarters.
#the data is smaller because is only where we have all three parameter information.
```
...
#more measurements on summer. high cloro on summer beacuse more light. More data from recent years. winter motnhs no data from recent years. Looks like we did more studies in winte rmotnhs before.
...
#also change in meas. freacuency. also recently only summer. Sechi is oposite of clorophil
...
#phospate is the readily available (disolved)

# you have to look at an specific lake. the relationship between 
```
         ...
         trophic.class = 
            ifelse(TSI.chl < 40, "Oligotrophic", 
                   ifelse(TSI.chl < 50, "Mesotrophic",
                          ifelse(TSI.chl < 70, "Eutrophic", "Hypereutrophic"))))  #rounds to without decimals becuase they dont meana anything

# they should be the same three numbers. Sometimes the result comflict. Using more than one parameters is sometimes not the best solution. Here we use the category Clorophyl to categorize. Sechi has human error. Of the three is the more direct method of what we are measuring. 

#Up to here 9 - 25 - 2019
...
# we see more points below the line. Clorophil give us a higher degree of heutrophic state than Sechi
...
#Similar relationship. So P give us lower values than Chlorofil. Bif diff in categorization. In those cases Clorofil are much higher that we would expect based on total p concentration.
```
...
#Violin give us good distribution information. 

#The rectangle represent the diff trophic states.(19 is 17 states plus Na plus some space), The horizontal lines are an alternative to the rectangles.
...
#keep an eye on seasonality. Indiana only monitors in the summer. Some states more mixed value. Is important the base line. Importnat hohw we see things are moving or changing. Some are eutrophic per se. 
...
#Save the Lagos Trophic as CSV


```{r}
write.csv(LAGOStrophic, "./Data/Processed/LAGOStrophic.csv", row.names = FALSE)
```
