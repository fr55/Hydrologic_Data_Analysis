...
> significantly faster than daily. Flash floods. Storms. 
Temp> big daily changes (oxygen, solubility that depends on T and reactions)
Fast storms/flash floods
Contaminent: can fluctuate a lot.

```{r}
#to look what rivers are abailable
a<-whatNWISsites(parameterCD="00060", statecd="NC")

library(sf)
library(mapview)
a_sf <- st_as_sf(a,coords = c('dec_long_va','dec_lat_va'),crs=4326)
mapview(a_sf)
```
...
dischargedatTFC2018 <- readNWISuv(
  ...
  renameNWISColumns() #This renames the columns!!!
...
#Datetime is not a Date is a POSIXct (number of seconds since a definied period of time) UTC(universal time coordinated. Time in Greenwich)
...
#Plotted in logscale. Interested in separating baseflow and quick flow.

#Overland flow  (Quick Flow)
###Pipes
###Sewers
#Groundwater     (Baseflow)
###Diff aquifers.
#Rain fall directly in the river channel. (Quick Flow)

#Storms are peak on discharge

#The base flow would follow the drops. The minimum values.
...
TFC2018 <- cbind(dischargedatTFC2018, TFCbaseflow) #put back the time info
...
#We saw how quick and base flow behave on storms. 
#Area under the curve is the amount of water (volume). Here ussually the area under the quick flow curve is higher because is urban.
...
#the NA_real_ is becuase the diff function gives you one result less. It adds an NA value.  The diff is the next diff.

#in the summary we take away the NA. 

PErcentBaseflow <- Export[1]/Export[3]*100
PErcentBaseflow  

PErcentQflow <- Export[2]/Export[3]*100
PErcentQflow  
...
> Baseflow 51.9%. Quickflow 48.1%

Repeat this analysis for Ellerbe Creek. How do the percentages compare?

> Baseflow 43.4%. Quickflow 56.6%

> Both urban streams. EC more urbanized.

```{r baseflowseparationEC}
...
EC2018 <- cbind(dischargedatEC2018, ECbaseflow) #put back the time info
...
PErcentBaseflowEC <- ExportEC[1]/ExportEC[3]*100
PErcentBaseflowEC  

PErcentQflowEC <- ExportEC[2]/ExportEC[3]*100
PErcentQflowEC  
...
> URban streams deeper (ensides) erotion, water quality.
...
> Normalize to take in to acount concentration time of big and small catchments. You could also try to find a way to normalyze by cathment shape

```{r Flashiness}
TFCsitedata <- readNWISsite(site = "02097280") #to see what info we have
...
> EC has flashier changes in hydrology. Flashy. How quickly the flow responds to events.

> Quick flow responds faster. Base flow slower.
