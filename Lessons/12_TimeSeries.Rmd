...
### Step trend analysis (a policy that started, could have a lag)
...
> aggregation: e.g. Put daily data to week data. e.g. Average values across a month. e.g. 15 minute data that you use to have a daily range.

> subsampling: Cutting your data. You have different frecuencies in diff periods of time. Biweekly data versus a year that has a lot of data. Low frecuency you dont get as much info and detail. You want equaly spaced.

> interpolation: Filling in gaps in your data set. there are methods for that. 
...
* **Seasonal Mann-Kendall**: seasonality, non-parametric, no temporal autocorelation, identical distribution (exact same frecuency). Function: `smk.test` (package: trend)
...
#Recomen: You look at the data before chossing test and everything.

# there is a big outlier. we dont needed it. There is a gap on sampling. We remove the four points. To few points. not useful info. We can have the question to be after 88. There is seasonality. 
...
Mendotadata <- Mendotadata %>%
  ...
  arrange(sampledate) #dyplier: sort it by date.
...
#many lakes dont have data over the winter. This lake has very nice data.
```
...
* **Piecewise constant**: <at any point in time you tak ethe nearest neighbor. looks like a ladder>
* **Linear**: <average. connect the dots>
* **Spline**: <for aquatic ussualy this dont outperforms linear. >
...
linearinterpolation <- as.data.frame(approx(Mendotadata, n = 303, method = "linear")) #we tell the function how many months we want to get. 
# the as data is because the function doesnt provide a datframe directly
#gives dates on units of nymbers of days since 1970.
...
# never will go above of below the actual data. Limitation of the method.

# Generate time series (smk.test needs ts, not data.frame)
Mendotatimeseries <- ts(linearinterpolation$TP, frequency = 12, 
                        start = c(1988, 7, 5), end = c(2013, 8, 5)) #the seasons are all the januaries and novembers etc. not the year.
# Run SMK test
...
# z = -1.4483 (direction and magnitud of the trend), p-value = 0.1475 (there is not a significant trend)
# alternative hypothesis: true S is not equal to 0
# sample estimates:
#     S  varS 
#  -218 22450 
#S counts the diff between one point and the next. a larger S is bigger difference.


summary(Mendotatrend)

#this analizes every month. (this is the 5 year thing. you need more data)
# Season 10:   S = 0 -164 1833.3 -0.547 -3.807 0.00014074 ***  October
# Season 11:   S = 0 -114 1833.3 -0.380 -2.639 0.00831237  **  November
...
> There is not a significatn overall trend but there is a trend decreasing in October and November.
...
# we want below 5. it is stationary.
```
...
# we were looking at some plots in a web page. We didnt get a distinct math to the spected apearence of the data. To the human eye is hard to tell.
```
...
auto.arima(ClearCreek_ts, trace = TRUE) #trace show you what is doing
...
#the projected area is pretty uniform but fits very well a typical year. You ocould plot one of the other models.
```
