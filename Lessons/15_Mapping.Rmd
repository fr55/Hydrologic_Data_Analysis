...
* **Point** <You have two measurements or z. No area. No dimention. you can use them to represent diff things>
* **Line** <Conection between points. The second point will also have a lat, long.>
* **Polygon** <Area. Series of lines, closed.>
...
secchi.summary <- LAGOScombined %>%
  ...
  summarise(secchi.mean = mean(secchi),
            ...
            depth = mean(maxdepth),
            lat = mean(nhd_lat), #way of including these data
            long = mean(nhd_long)) %>%
  drop_na()
```
...
#get an idea os what your data looks like. Not statistical info but valuable.
...
# visualize state plot
LAGOSMaineplot <- ggplot(states.subset.maine) +
  geom_sf(fill = "white")
print(LAGOSMaineplot)

LAGOScombined.maine <- filter(LAGOScombined, state == "ME")

maine.secchi.summary <- LAGOScombined.maine %>%
  group_by(lagoslakeid) %>%
  summarise(secchi.mean = mean(secchi),
            area = mean(lake_area_ha),
            depth = mean(maxdepth),
...
secchi.spatial.maine <- st_as_sf(maine.secchi.summary, coords = c("long", "lat"), crs = 4326)
...
```{r}
plot_grid(Secchiplot.maine, Secchiplot.maine2)

# she polted lake size to lake depth.
```
