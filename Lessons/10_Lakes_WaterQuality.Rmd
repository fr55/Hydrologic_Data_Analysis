...
> Variable 1: Land Use (21, 23)

> Variable 2: Atmospheric Deposition of N

> Variable 3: Chlorophyl

> Variable 4: LULC 500m CAFOs (LULC 23)

> Variable 5: Secchi Depth
...
LAGOS.lulc <- LAGOSdata$buffer500m.lulc
LAGOS.chag <- LAGOSdata$hu8.chag
...
LAGOS.lulc$lagoslakeid <- as.factor(LAGOS.lulc$lagoslakeid)

LAGOS.chag$lagoslakeid <- as.factor(LAGOS.chag$hu8_zoneid)
...
LAGOSTPandTN <- left_join(LAGOSnutrient, LAGOSlocations, by = "lagoslakeid")
LAGOSTPandTN <- left_join(LAGOSTPandTN, LAGOS.lulc, by = "lagoslakeid")
LAGOSTPandTN <- left_join(LAGOSTPandTN, LAGOS.chag, by = "lagoslakeid")

LAGOStotal <- select(LAGOSTPandTN, lagoslakeid, sampledate, chla, tp, tn, secchi, gnis_name, lake_area_ha, state, state_name, buffer500m_nlcd2011_pct_21, buffer500m_nlcd2011_pct_23, hu8_dep_totaln_2010_mean) %>%
  drop_na()
