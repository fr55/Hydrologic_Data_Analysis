...
> how quickly and how ... it will change
...
> CQ plot. Concentration Discharge. Things We care about: the slope. no slope chemostatic. Neg slope. diluting. Positive slope:Flushing (the chem its being flushed in to the stream)

> the sources over the storm come from diff path ways (sources). histerecis.
...
#clock and anticlok wise histerecis

# Is diluting. Anticlockwize. The rising leam of the storm has a lower concentration that the one that is going down (the falling leam).

#in this cases it is not posible to estimate concentration with flow.

#concentrattion of nitrogen are lower in the quick pathways. The nitrogen is in the groundwater. not in the surface of the soil.

#Questions: When the Conc are higher? is it diluting?

#Diluting storms. what means for the system. You are adding water but no more nutrients. 

#The diluting anticlock is more common.
...
> this are the ways hidrology can impact water quality.

## Using dissolved oxygen to estimate metabolism

What is "ecosystem metabolism"?

> The rate that an ecosystem consumes energy

Why would we want to know the *GPP* and *ER* of a river reach?

> GPP anr ER (Ecosystem Respiration)

> we have biomes. Can we clasify streams in similar ways.
...
Eno.lon <- Enodat[[2]]$lon  #the double bracket is how to call objects from a list.
...
#the convert_UTC_to_solartime() function is the thing that needed streampulse.

ggplot(Eno.DO, aes(x = DateTime_Solar, y = DO_mgL)) + geom_line()

#net heterotrophic. Eno produces CO2.
